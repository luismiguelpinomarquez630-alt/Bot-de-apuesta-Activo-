...
# --- obtener_partidos (mockeado, ningún test toca la red) -----------------
...
def test_obtener_partidos_count_cero_sin_items():
    ...
    with patch.object(cliente_1x, "_get", return_value=respuesta) as mock_get:
        resultado = cliente_1x.obtener_partidos(champ_id=2664249, desde=1784792100, hasta=1784878500)
...
def test_obtener_partidos_alinea_timestamps_antes_de_pedir():
    respuesta = {"count": 0}
    with patch.object(cliente_1x, "_get", return_value=respuesta) as mock_get:
        cliente_1x.obtener_partidos(champ_id=1, desde=1784803600, hasta=1784890000)
...
def test_obtener_partidos_rechaza_ventana_mayor_a_86400s():
    with pytest.raises(ValueError):
        cliente_1x.obtener_partidos(champ_id=1, desde=0, hasta=200_000)
